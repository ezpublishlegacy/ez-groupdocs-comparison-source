...
{ 
    ...
    { 
        return array( 'fields' => array( 'id' => array( 'name' => 'ID', 
                      ...
                      'keys'=> array( 'id' ), 
                      'function_attributes' => array('embed_key_object' => 'getEmbedKeyObject', 'file_id_object' => 'getFileIdObject' ), // accessing to attribute "user_object" will trigger getUserObject() method
                      'increment_key' => 'id', 
    ...
    { 
        $embed_key = $this->attribute('embed_key');
     ...
     * @param int $user_id
     * @param string $value
     * @return GroupDocsComparison
     */
    public static function create($embed_key, $file_id, $file_hook, $width, $height )
    { 
        $row = array( 'id'      => null, 
                      'embed_key' => $embed_key,
                      'file_id' => $file_id, 
    ...
    /**
     * Shows the data as GroupDocsComparison with given id
    ...
    { 
        ...
        if ( $result instanceof GroupDocsComparison ) 
            return $result; 
        else 
            return false; 
    }
    ...
    /**
     * Shows all the objects GroupDocsComparison as object or array
     * @param int $asObject
     * @return array( GroupDocsComparison )
     */ 
    ...
    { 
        $db = eZDB::instance(); 
        $query = 'SELECT COUNT(id) AS count FROM gdc'; 
    ...
    { 
        $db = eZDB::instance(); 
        $query = 'SELECT MAX(id) AS mid FROM gdc'; 
        $rows = $db -> arrayQuery( $query ); 
        return $rows[0]['mid'];
    } 
